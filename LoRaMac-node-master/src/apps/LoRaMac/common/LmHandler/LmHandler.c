...
uint8_t LmHandlerStartCad( uint8_t DR )
{
    return CadOnChannel(0, DR);
}
...
static void McpsConfirm( McpsConfirm_t *mcpsConfirm )
{
    ...
    {
        printf("TX: %ld, RX: %ld\n",TxParams.UplinkCounter-1, RxParams.DownlinkCounter);
...
// custom send
LmHandlerErrorStatus_t LmHandlerSend_custom( LmHandlerAppData_t *appData, int8_t DR )
{
    LoRaMacStatus_t status;
    McpsReq_t mcpsReq;
    LoRaMacTxInfo_t txInfo;
