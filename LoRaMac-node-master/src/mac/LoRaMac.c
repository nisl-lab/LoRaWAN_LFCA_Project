...
static LoRaMacStatus_t ScheduleTx_custom( bool allowDelayedTx );
...
/*!
 * Structure used to store the radio Cad event data
 */
struct
{
    TimerTime_t CurTime;
...
static void OnRadioCadDone( bool channelActivityDetected )
{
    CadDoneParams.CurTime = TimerGetCurrentTime( );
    CadDoneParams.channelActivityDetected = channelActivityDetected;
    LoRaMacRadioEvents.Events.CadDone = 1;
    OnMacProcessNotify( );
}
...
static void ProcessRadioCadDone( void ){
    Radio.Sleep( );
    if(CadDoneParams.channelActivityDetected){
        // printf("%d\n",CadDoneParams.CurTime);
        // printf("detecteddetecteddetecteddetecteddetected!!!!\n");
    }
    // }else{
    //     printf("no!!\n");
    // }
...
LoRaMacStatus_t Send( LoRaMacHeader_t* macHdr, uint8_t fPort, void* fBuffer, uint16_t fBufferSize )
{
    ...
    if( ( status == LORAMAC_STATUS_OK ) || ( status == LORAMAC_STATUS_SKIPPED_APP_DATA ) )
    {
        ...
        status = ScheduleTx_custom( false );
    }
...
uint8_t CadOnChannel( uint8_t channel, uint8_t DR ){
    uint32_t tickstart = HAL_GetTick();
    float maxEIRP = MIN( Nvm.MacGroup2.MacParams.MaxEirp, 14.0 );
    int8_t txPowerLimited = RegionCommonLimitTxPower(Nvm.MacGroup1.ChannelsTxPower, RegionBands[0].TxMaxPower );
    int8_t phyTxPower = RegionCommonComputeTxPower( txPowerLimited, maxEIRP, Nvm.MacGroup2.MacParams.AntennaGain );
    uint8_t state = 0;
    int8_t DRs[] = {0,0,0,9,9,7};
    int8_t phyDr = DRs[DR];
    Radio.SetChannel(922100000);
    Radio.SetTxConfig(MODEM_LORA, phyTxPower, 0, 0, phyDr, 1, 2000, false, true, 0, 0, false, 4000) ;
    Radio.SetRxConfig(MODEM_LORA, 0, phyDr, 1, 0, 2000, 41, false, 0, true, 0, 0, true, false);
    Radio.SetMaxPayloadLength( MODEM_LORA, 242 + LORAMAC_FRAME_PAYLOAD_OVERHEAD_SIZE );

    uint16_t symbolTime = 0;
    if(phyDr == 9){
        // symbolTime = 4352;
        symbolTime = 435;
    }
    ...
    symbolTime += 30;
    Radio.StartCad();
    Delay10Us(symbolTime);
    int rssi = Radio.Rssi(MODEM_LORA);
    
    while(!LoRaMacRadioEvents.Events.CadDone);
...
LoRaMacStatus_t LoRaMacInitialization( LoRaMacPrimitives_t* primitives, LoRaMacCallback_t* callbacks, LoRaMacRegion_t region )
{
    ...
    MacCtx.RadioEvents.CadDone = OnRadioCadDone;
